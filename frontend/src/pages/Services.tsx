...
import { Wrench, Plus, ArrowLeft, CheckCircle } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {showForm && (
          <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-6 shadow-sm">
            ...
            {errors.name && <p className="text-sm text-red-600 mb-1">{errors.name}</p>}
            {errors.price && <p className="text-sm text-red-600 mb-1">{errors.price}</p>}
            <div className="flex gap-2"><button onClick={handleSave} disabled={saving} className="px-4 py-2 bg-brand-900 text-white rounded-lg font-medium text-sm">{saving?'Saving...':'Save Service'}</button><button onClick={()=>setShowForm(false)} className="px-4 py-2 bg-slate-100 rounded-lg font-medium text-sm">Cancel</button></div>
          </div>
        )}
