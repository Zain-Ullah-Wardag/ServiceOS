...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // If orderId not provided (wizard new order), create Order atomically
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (measurementId) {
          ...
          // Measurement must belong to selected customer (model has customerId?)
          if (measurement.customerId && measurement.customerId !== customerId) {
            throw new Error('Measurement customer mismatch');
          }
        }
